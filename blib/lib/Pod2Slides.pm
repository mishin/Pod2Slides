...
__PACKAGE__->mk_accessors(
    qw(file podfile poddir cnt node opts thistext oldtext  tt root meta title head_level current_heading boilerplate has_locallib)
);
        ...
        Template->new(
            {
                ...
                ENCODING     => 'utf-8',
                INCLUDE_PATH => [
                    catdir( File::HomeDir->my_home, 'pod2slides' ),
...
sub stringify_node {
    ...
    return "cannot stringify node type $type"
      unless $type =~ /head/ || $type eq 'Para' || $type =~ /^item/;
...
sub handle_for_include_code {
    ...
    $me->thistext( "<pre><font color='#aa0000'>file: "
          . $me->node->[2][2]
          . "</font>\n<div  style='font-size:$fontsize" . q{%'>}
          . "$inced</div></pre>" );
    $me->write_slide;
}
...
sub handle_for_run_code {
    ...
    my $perl = $noperl ? '' : 'perl ';
    $me->rawtext( $dir . "/" . $file . ":\n" . $run );
    $me->thistext( "<pre><font color='#00cc00'>~/$dir\$</font> " 
          . $perl
          . "$file $args\n$run</pre>" );
    $me->write_slide;
}
...
sub handle_for_include_and_run {
    ...
    $me->rawtext( $codefile . ":\n" . $inced );
    $me->thistext(
        "<pre><font color='#aa0000'>file: $codefile</font>\n$inced</pre>");
    $me->write_slide;

    $me->inc_cnt;

    my ( $dir, $file, $run, $noperl ) =
      $me->run_file( $codefile, join( ' ', @args ) );
    $me->rawtext( $dir . "/" . $file . ":\n" . $run );
    my $perl = $noperl ? '' : 'perl ';
    $me->thistext( "<pre><font color='#00cc00'>~/$dir\$</font> " 
          . $perl
...
sub highlight {
    ...
    $text =~ s/</&lt;/g;
    $text =~
s{^(\s+)([~/](.*?)\$)(.*)}{$1<font color='#00cc00'>$2</font><font color='#cc0000'>$4</font>}mg;
